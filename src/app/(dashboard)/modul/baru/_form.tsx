...
export function BuatModulForm({
  ...
  async function onSubmit(data: FormValues) {
    ...
    try {
      ...
      const generatePromise = fetch(`/api/modul/${modul.id}/generate`, {
        method: "POST",
      });
  ...
  if (isProcessing)
    return (
      <ProcessingScreen steps={PROCESSING_STEPS} currentStep={currentStep} />
    );
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <select
                    {...register("menit", { valueAsNumber: true })}
                    className="h-10 px-2 rounded-md text-sm border border-stone-200 bg-white focus:outline-none focus:border-teal-600 flex-1"
                  >
                    {[30, 35, 40, 45, 50, 60, 80, 90].map((m) => (
                      <option key={m} value={m}>
                        {m} mnt
                      </option>
                    ))}
                ...
                {showExtra && (
                  ...
                  >
                    ...
                    <p className="text-2xs text-stone-400 mt-1">
                      Kosongkan jika tidak yakin — AI akan mengisi dari database
                      kurikulum.
                    </p>
                  </motion.div>
                )}
        ...
        <div className="sticky bottom-0 bg-stone-50 border-t border-stone-200 -mx-8 px-8 py-4 mt-6 flex items-center justify-between">
          <p className="text-xs text-stone-400">
            Field bertanda <span className="text-danger-bold">*</span> wajib
            diisi
          </p>
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="md" type="button" onClick={() => router.back()}>
              Batal
            </Button>
            <Button
              type="submit"
